    ...
    <div className="flex-1 flex flex-col relative h-full">
      <div className="p-6 pb-2 bg-indigo-600 z-10 sticky top-0 shadow-md flex flex-col space-y-4">
        ...
        <div className="bg-indigo-900/40 p-3 rounded-2xl border-2 border-dashed border-indigo-500/50 flex flex-col items-center justify-center">
          <span className="text-[10px] font-black text-indigo-300 uppercase tracking-widest">{roomState.secretCategory || roomState.settings.category}</span>
          <span className="text-lg font-black text-white">{roomState.myWord || roomState.myHint || (me?.isSpectator ? "Spectating" : "You are the Impostor")}</span>
        </div>
        ...
        <AnimatePresence initial={false}>
          {chatMessages.map((msg) => {
            ...
            return (
              ...
              >
                {!isMe && (
                  <div className={cn("w-10 h-10 rounded-full border-2 border-white flex items-center justify-center text-lg shrink-0 shadow-inner", p?.color)}>
                    {p?.avatar}
                  </div>
                )}
                <div className={cn(
                  ...
                  {!isMe && <div className={cn("text-[10px] font-black mb-0.5 uppercase tracking-wider", textColorClass)}>{p?.name}</div>}
      ...
      <div className="absolute bottom-0 left-0 right-0">
        <div className="absolute right-4 bottom-full mb-4 z-20">
          <button
            onClick={() => toggleSkipDiscussion()}
            className={cn("px-4 py-3 rounded-3xl flex items-center space-x-2 text-xs font-black uppercase tracking-widest transition-transform active:translate-y-1 shadow-2xl border-b-4", 
               me?.wantsToSkipDiscussion ? "bg-yellow-400 border-yellow-600 text-indigo-900" : "bg-pink-500 border-pink-700 text-white hover:bg-pink-600"
            )}
          >
            <span>Skip Discussion</span>
            <div className="bg-black/20 px-2 py-0.5 rounded-full">{roomState.players.filter(p => p.wantsToSkipDiscussion).length}/{roomState.players.filter(p => !p.isSpectator).length}</div>
            <FastForward className="w-4 h-4" />
          </button>
        </div>
        <div className="p-4 bg-indigo-600 border-t-4 border-indigo-500/30">
          {isSpectator ? (
          <div className="bg-indigo-700/50 rounded-full px-5 py-3 text-center border-4 border-indigo-500/30">
            <span className="text-indigo-200 font-bold text-sm uppercase tracking-widest">Spectators cannot chat</span>
          </div>
        ) : (
...
function CluesDropdown({ clues, players }: { clues: any[], players: any[] }) {
  ...
  const cluesByRound = clues.reduce((acc, clue) => {
                ...
                <div key={round} className="space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {cluesByRound[round].map((c: any, i: number) => {
                      ...
                      if (!p) return null;
                      return (
                        <div key={i} className="bg-indigo-800/40 rounded-xl p-2 flex items-center space-x-2 border border-indigo-500/30">
                          <div className={cn("w-6 h-6 rounded-full flex items-center justify-center text-xs shadow-inner", p.color)}>
                            {p.avatar}
                          </div>
                          <div className="flex flex-col min-w-0">
                            <span className="text-[9px] font-black text-indigo-300 uppercase tracking-widest leading-tight truncate">{p.name}</span>
                            <span className={cn("text-xs font-bold uppercase tracking-wider truncate", c.clue === '- Skipped -' ? "text-red-400" : "text-white")}>{c.clue}</span>
                          </div>
                        </div>
                      );
                    })}
