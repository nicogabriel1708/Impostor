...
const COLORS = [
  'bg-red-500', 'bg-orange-500', 'bg-yellow-400', 'bg-lime-400', 'bg-green-700', 'bg-cyan-500',
  'bg-sky-400', 'bg-blue-700', 'bg-pink-400', 'bg-rose-600', 'bg-fuchsia-400', 'bg-purple-700',
  'bg-amber-600', 'bg-amber-900', 'bg-gray-300', 'bg-gray-600', 'bg-white', 'bg-black'
];
    ...
    <div className="flex-1 flex flex-col h-full relative overflow-y-auto">
      ...
      <div className="flex-1 flex flex-col pb-6 min-h-max">
        <div className="text-center mt-6 mb-4 shrink-0 px-6 pt-12">
          <h2 className="text-xs font-bold tracking-widest text-indigo-300 uppercase">Room <span className="text-yellow-400 ml-1">{roomCode}</span></h2>
          <h1 className="text-3xl font-black mt-1 tracking-tighter uppercase italic text-white">Who are you?</h1>
        </div>

        <form id="setup-form" onSubmit={handleSubmit} className="flex flex-col flex-1 px-6 space-y-5">
          <div className="flex flex-col shrink-0">
            <label className="text-xs font-bold text-indigo-200 uppercase tracking-widest pl-1 mb-2">Display Name</label>
            <input
              type="text"
          ...
          <div className="flex flex-col">
            <label className="text-xs font-bold text-indigo-200 uppercase tracking-widest pl-1 mb-2 shrink-0">Color</label>
              ...
              {visibleColors.map(c => (
                <button
                  ...
                  className={cn(
                    ...
                    c,
                    color === c ? "border-white/50 ring-4 ring-offset-2 ring-offset-indigo-600 ring-yellow-400 border-b-0 translate-y-1 z-10" : "border-black/20"
                  )}
                />
              ))}
            </div>
          ...
          <div className="flex flex-col">
            <label className="text-xs font-bold text-indigo-200 uppercase tracking-widest pl-1 mb-2 shrink-0">Avatar</label>
            <div className="grid grid-cols-6 gap-4 sm:gap-5 px-3 py-4 -mx-3">
