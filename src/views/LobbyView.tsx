  ...
  return (
    <div className="flex-1 flex flex-col p-6 overflow-y-auto relative">
      ...
      <div className="flex flex-col items-center mb-4 mt-12">
        ...
        >
          <span className="text-4xl font-mono font-black tracking-[0.2em] text-yellow-400 ml-[0.2em]">{roomState.code}</span>
      ...
      <div className="grid grid-cols-4 gap-x-3 gap-y-10 mb-8">
        {roomState.players.map(p => (
          <motion.div 
            ...
            key={p.id} 
            className={cn("flex flex-col items-center space-y-2 relative", menuPlayerId === p.id ? "z-50" : "z-10")}
          >
            <button 
              onClick={(e) => { e.preventDefault(); e.stopPropagation(); if (isHost && p.id !== sessionId) setMenuPlayerId(p.id); }}
              className={cn(
                ...
                (isHost && p.id !== sessionId) ? "hover:scale-105 active:scale-95 cursor-pointer" : "cursor-default"
              )}
            >
              {p.avatar}
              {p.isHost && (
            ...
            <span className="text-xs font-bold truncate w-full text-center text-indigo-100">
              {p.name}
            </span>
        ...
        {Array.from({ length: Math.max(0, 3 - roomState.players.length) }).map((_, i) => (
          <div key={`empty-${i}`} className="flex flex-col items-center space-y-2 opacity-40">
             <div className="w-16 h-16 rounded-[20px] border-2 border-dashed border-indigo-400/50 bg-indigo-900/20 flex items-center justify-center">
             </div>
             <span className="text-xs font-bold truncate w-full text-center text-indigo-300">Waiting...</span>
          </div>
        ))}
      </div>
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            {isHost ? (
              ...
              <div className="h-3 bg-indigo-100 rounded-full overflow-hidden shadow-inner">
                 <div className="h-full bg-yellow-400" style={{ width: `${(roomState.settings.clueTimeLimit / 120) * 100}%`}}></div>
              </div>
            )}
          </div>
          ...
          <div>
            ...
            {isHost ? (
              ...
              <div className="h-3 bg-indigo-100 rounded-full overflow-hidden shadow-inner">
                 <div className="h-full bg-yellow-400" style={{ width: `${(roomState.settings.discussionTimeLimit / 300) * 100}%`}}></div>
              </div>
            )}
          </div>
          ...
          <div className="space-y-3">
            ...
            {isHost ? (
              <div className="grid grid-cols-2 gap-3">
                ...
                >
                  <div className={cn("text-sm font-black uppercase tracking-widest mb-1", roomState.settings.roundMode === 'short' ? "text-indigo-900" : "text-indigo-900")}>Short</div>
                  <div className={cn("text-xs font-bold leading-tight", roomState.settings.roundMode === 'short' ? "text-indigo-800" : "text-indigo-400")}>One vote only.</div>
                </button>
                <button
                ...
                >
                  <div className={cn("text-sm font-black uppercase tracking-widest mb-1", roomState.settings.roundMode === 'long' ? "text-indigo-900" : "text-indigo-900")}>Long</div>
                  <div className={cn("text-xs font-bold leading-tight", roomState.settings.roundMode === 'long' ? "text-indigo-800" : "text-indigo-400")}>Play until impostors win/lose.</div>
                </button>
              </div>
            ) : (
              <div className="w-full bg-indigo-50 border-2 border-indigo-100 rounded-xl px-4 py-3 shadow-inner flex flex-col">
                 <span className="font-bold text-indigo-900 uppercase tracking-widest text-sm">{roomState.settings.roundMode === 'short' ? 'Short' : 'Long'}</span>
                 <span className="text-xs text-indigo-400 font-bold">{roomState.settings.roundMode === 'short' ? 'One vote only.' : 'Play until impostors win/lose.'}</span>
              </div>
            )}
          </div>
          ...
          <div>
            <label className="text-xs font-black text-indigo-400 uppercase tracking-widest flex justify-between items-center mb-3">
              ...
              {isHost && (
                <button
                  onClick={() => updateSettings({ hintMode: roomState.settings.hintMode === 'none' ? 'vague' : 'none' })}
                  className={cn(
      ...
      <AnimatePresence>
        {menuPlayerId && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-6">
            ...
            >
              <div className={cn("w-20 h-20 text-4xl rounded-[24px] flex items-center justify-center mb-4 shadow-inner text-white border-4 border-white/20", roomState.players.find(p => p.id === menuPlayerId)?.color)}>
