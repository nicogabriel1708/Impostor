...
export function RevealView() {
  ...
  const bgColor = isContinues ? "bg-indigo-600" : didTie ? "bg-yellow-400" : didPlayersWin ? "bg-indigo-500" : "bg-pink-500";
    ...
    <div className="flex-1 flex flex-col h-full overflow-y-auto">
      <div className={cn("p-8 pb-12 flex flex-col items-center justify-center text-center shadow-md", bgColor)}>
        ...
        >
          ...
          <h1 className={cn("text-5xl font-black mb-2 uppercase italic tracking-tighter", textColor)}>{title}</h1>
          {!isContinues && (
            <p className={cn("text-lg font-bold uppercase tracking-wider", didTie ? "text-indigo-800" : "text-white/80")}>
      ...
      <div className="p-6 space-y-8 flex-1">
            ...
            {result.impostors.map(impId => {
              ...
              return (
                <div key={impId} className={cn(
                  ...
                  {eliminated && <div className="text-xs font-black text-pink-500 uppercase tracking-widest">Eliminated</div>}
                </div>
              );
            })}
          </div>
        ...
        <div className={cn("space-y-4", !isContinues && "pt-6 border-t-4 border-indigo-500/20")}>
          <h3 className="text-sm font-black text-indigo-400 uppercase tracking-widest mb-4">Voting Results</h3>
          <div className="space-y-3">
            {result.votes.sort((a, b) => b.voterIds.length - a.voterIds.length).map((v, i) => {
              const target = v.playerId === 'skip' ? { name: 'Skipped', avatar: '⏭️' } : roomState.players.find(p => p.id === v.playerId);
                ...
                <div key={v.playerId} className={cn(
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                      ...
                      <div className={cn("font-black text-lg uppercase tracking-wider", isEliminated ? "text-pink-900" : "text-indigo-900")}>{target.name}</div>
                    </div>
                    <div className={cn("flex items-center space-x-1 px-3 py-1 rounded-full text-sm font-black uppercase tracking-widest shadow-inner", isEliminated ? "bg-pink-200 text-pink-900" : "bg-indigo-100 text-indigo-600")}>
                  ...
                  <div className="flex flex-wrap gap-2 pl-12">
                    {v.voterIds.map(voterId => {
                      ...
                      return voter ? (
                        <div key={voterId} className="flex items-center space-x-1 bg-indigo-50 border-2 border-indigo-100 rounded-full px-2 py-0.5 text-xs font-bold text-indigo-500 shadow-sm">
      ...
      <div className="p-6 pt-0 mt-auto">
        {isContinues ? (
          <div className="w-full bg-indigo-900/40 text-indigo-200 font-bold flex flex-col items-center justify-center py-5 px-6 rounded-[20px] border-4 border-dashed border-indigo-500/50 uppercase tracking-widest text-sm space-y-2">
            ...
            <div className="scale-125"><Timer endsAt={roomState.timerEndsAt} /></div>
          </div>
        ) : isHost ? (
          <button
            onClick={nextRound}
