      ...
      <div className="p-6 pb-4 bg-indigo-600 z-10 sticky top-0 shadow-md">
        <div className="flex justify-between items-center bg-indigo-700/50 p-4 rounded-3xl border-2 border-indigo-500/30">
          <div className="flex flex-col">
            ...
            {me?.role === 'player' && (
              <span className="text-xs font-bold text-yellow-300 mt-1 uppercase tracking-widest">Word: <span className="text-white font-black">{roomState.myWord}</span></span>
            )}
          </div>
            ...
            return (
              <motion.div 
                ...
                className={cn(
                  ...
                  c.playerId === sessionId ? "bg-indigo-500 border-indigo-400" : "bg-white border-indigo-100 text-indigo-900"
                )}
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className={cn("text-xs font-black uppercase tracking-widest mb-1", c.playerId === sessionId ? "text-indigo-200" : "text-indigo-400")}>{p?.name}</div>
                  <div className={cn("text-xl font-black break-words", c.playerId === sessionId ? "text-white" : "text-indigo-900")}>{c.clue}</div>
                </div>
              </motion.div>
            );
          })}
        ...
        <div className="bg-white rounded-[30px] p-5 shadow-2xl border-4 border-indigo-200">
          {isMyTurn ? (
            <form onSubmit={handleSubmit} className="flex flex-col space-y-3">
              <label className="text-xs font-black text-pink-500 uppercase tracking-widest pl-1">Your Turn: Enter a clue</label>
