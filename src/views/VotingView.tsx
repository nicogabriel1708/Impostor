      ...
      {hasVoted || isSpectator ? (
        ...
        <>
          <div className="bg-indigo-800/40 border-2 border-indigo-500/50 p-4 rounded-[20px] mb-4">
            <div className="text-xs font-black text-indigo-300 uppercase tracking-widest mb-1">Your Clue</div>
            <div className="text-xl font-black text-white">
              {[...roomState.clues].reverse().find(c => c.playerId === sessionId && c.round === roomState.roundCount)?.clue || 'No clue given'}
            </div>
          </div>
          ...
          {roomState.players.map(p => {
            ...
            // Find their clue
            const theirClue = [...roomState.clues].reverse().find(c => c.playerId === p.id && c.round === roomState.roundCount)?.clue || 'No clue given';
            ...
            return (
              ...
              >
                <div className={cn("w-14 h-14 rounded-[16px] flex items-center justify-center text-3xl shrink-0 group-hover:scale-110 transition-transform shadow-inner border-2 border-white/20", p.color)}>
                  {p.avatar}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="font-black text-xl text-indigo-900 truncate uppercase tracking-wider">{p.name}</div>
                  {theirClue === '- Skipped -' ? (
                    <div className="text-sm text-red-500 font-black uppercase tracking-widest truncate">{theirClue}</div>
                  ) : (
